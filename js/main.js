...
function addUserToUserList(user, date_time, text) {
    ...
    let date = new Date(date_time);
    let date_string = '';
    if(date_time != null) date_string = parseDate(date);
...
function refreshUsersSections() {
    ...
    let full_users = JSON.parse(data.getItem('users') )
    console.log('full users =  ', full_users);
    console.log(' users arr =  ', users_array);
    for (let i = 0; i < full_users.length; i++) {
        let flag = false;
        if( full_users[i]['login'] != currentUser){
            
            users_array.forEach(item => {
                if (item['user'] == full_users[i]['login']) flag = true;                
            });

            if(!flag)
            {
                console.log('i =   ',full_users[i]['login']);

                addUserToUserList(full_users[i]['login'], null, '')
            }
        }
    }
...
window.onload = () => {
    ...
    console.log(data.getItem('users'))
    clearChatSections();
    clearUsersSections();
    if (isLoggined) {
        refreshUsersSections();
